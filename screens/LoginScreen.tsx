...
import {
  ...
  saveSession, loadSession, Session, API_BASE, refreshSessionWithToken,
} from "../lib/api";
...
export default function LoginScreen({ onLogin }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setBioEn(en);
      // Auto-trigger Face ID on app open if enabled
      if (en) {
        setTimeout(async () => {
          // Authenticate first
          ...
          setLoading(true);
          try {
            // Try stored biometric refresh token first
            ...
            // Fall back to main stored session
            const existing = await loadSession();
            if (existing?.access_token) {
              onLogin(existing);
              return;
            }
            // Both failed - need to sign in with password once
            setLoading(false);
  ...
  async function handleBiometric() {
    ...
    try {
      ...
      const storedEmail = await getBiometricEmail();
      if (storedEmail) setEmail(storedEmail);
      setError("Please sign in with password to re-enable Face ID.");
    } catch {
      setLoading(false);
    }
  }
    ...
    <SafeAreaView style={s.safe}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.scroll}>
          ...
          {/* Biometric */}
          {biometricEnabled && mode === "signin" && (
            <TouchableOpacity style={s.bioBtn} onPress={handleBiometric} activeOpacity={0.85}>
