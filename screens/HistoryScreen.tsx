  ...
  async function deleteItem(id: string, type: string, name: string) {
    Alert.alert("Delete", `Remove "${name}" from history?`, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await fetch(`${API_BASE}/api/history?token=${token}&id=${id}&type=${type}`, { method: "DELETE" });
              ...
              <View style={s.limitBanner}>
                <Text style={s.limitTxt}>
                  {plan === "free"
                    ...
                    : plan === "seller"
                    ? `Seller plan: 10 Thrift Runs/month`
                    : "Unlimited Thrift Runs"}
                </Text>
              </View>
...
const s = StyleSheet.create({
  ...
  emptyBtn:      { backgroundColor:C.greenBg, borderRadius:12, paddingHorizontal:20, paddingVertical:12, borderWidth:1, borderColor:C.greenBorder },
  ...
  runItem:       { flexDirection:"row", alignItems:"center", paddingVertical:8, borderBottomWidth:1, borderBottomColor:C.border, gap:8 },
